=== WP Popup ===
Contributors: rxnlabs, dannycorner, rpasillas, kenjigarland, xavierserranoa
...
* Avoid a potential fatal error when Beaver Builder is active and another plugin or theme sets the `post__not_in` query var incorrectly (a rare situation).
* Fix a bug that could cause extra <p> tags to be added to popups built using the Block Editor when the 'Disable "the_content" filters' setting was enabled.
* Fix PHP deprecation warnings on servers running PHP 8.1 and above.

= 1.2.5 =
* Fix bug that could occasionally cause certain single posts to redirect to the homepage on sites without Beaver Builder.
